...
(() => {
  ...
  const CONFIG = {
    ...
    //   instead of redirecting (the screen + its markup/CSS are still
    //   fully in place, just unused while this is false).
    useQrRegisterScreen: false,
  };
  ...
  function measureStage() {
    ...
    baseRadius = (rect.width / 2) * 0.84;
  }

  // Flat 2D circular path — every item stays the same size and the same
  // opacity as it travels around the ring, so nothing reads as "near" or
  // "far". Only the trail glow responds to motion speed.
  ...
  async function runRegisterTransition() {
    ...
    // plasma sweeps in and fully covers the screen
    await tween(REDUCED_MOTION ? 250 : 600, (p) => setPlasma(Easing.easeInOutCubic(p) * 150));

    if (CONFIG.useQrRegisterScreen) {
      await sleep(120);
      setActiveScreen('register');
      AudioEngine.playChime();

      // plasma withdraws, revealing the register/QR screen underneath
      await tween(REDUCED_MOTION ? 250 : 600, (p) => setPlasma(150 - Easing.easeOutCubic(p) * 150));
      sequenceLocked = false;
      dsButtonEl.disabled = false;
    } else {
      // screen is fully covered — hand off to the real registration site
